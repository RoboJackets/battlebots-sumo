...
//Data structure to store past values
std::vector<int> r_past_values(10);
std::vector<int> l_past_values(10);
...
void getToF() {
    ...
    for (int i = 0; i < l_past_values.size(); ++i) {
      Serial.printf("%d ", r_past_values[i]);
    }
    Serial.println();
    ...
    for (int i = 0; i < l_past_values.size(); ++i) {
      Serial.printf("%d ", l_past_values[i]);
    }
    Serial.println();
...
void doFuzzy() {
    ...
    int prev_r = *(r_past_values.begin());
    for (auto i = r_past_values.begin() + 1; i != r_past_values.end(); ++i) {
      r_change += *i - prev_r;
      prev_r = *i;
    }

    int prev_l = *(l_past_values.begin());
    for (auto i = l_past_values.begin() + 1; i != l_past_values.end(); ++i) {
      l_change += *i - prev_l;
      prev_l = *i;
    }
